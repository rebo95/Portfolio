...
CollisionManager::CollisionManager(SDLGame* game, BulletsManager* bulletsManager, FightersManager* fighterManager, AsteroidsManager* astroidManger, Bonus* bonus):GameObject(game), bulletsManager_(bulletsManager),
fighterManager_(fighterManager), asteroidsManager_(astroidManger), bonus_(bonus)
{
}
...
void CollisionManager::update(Uint32 time) {
	...
	//colision de las balas con el Bonus
	for (GameObject* b : bullets) {
		if (Collisions::collidesWithRotation(bonus_, b) && !l) {
			if (static_cast<Bullet*>(b)->isActive() && static_cast<Bonus*>(bonus_)->isActive()) {
				Message* m = new Message{ BULLET_BONUS_COLLISION};
				send(m);
				delete(m);
				m = nullptr;
			}
